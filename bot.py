...
# Active Paper Trades track karne ke liye dictionary
active_paper_trades = {}

@app.route('/')
def index():
    return "Akash Paper Trading Sniper Bot Active Hai!"
...
def analyze_market_and_setup():
    try:
        ...
        if current_price > ema_200:
            ...
            sl = round(entry - 400, 2)
            tp = round(entry + 800, 2)  # 1:2 Risk Reward
            direction = "LONG"
        else:
...
def send_welcome(message):
    bot.reply_to(message, "🎯 **Akash Paper Trading Bot** active hai!\n\nCommands:\n/setup - Live Pro Setup dekho\n/paper - Virtual Trade start karo aur result track karo!")

@bot.message_handler(commands=['setup'])
def send_setup(message):
    bot.reply_to(message, "🔍 Pro-rules ke hisab se market scan ho raha hai... (200 EMA & Risk/Reward check)")
    
    price, ema, trend, entry, sl, tp, direction = analyze_market_and_setup()
    
    if price and isinstance(ema, float):
        reply_text = f"""📊 **PRO QUANT SETUP (PAPER TRADING)** 📊
...
🛑 **Stop Loss (1:2 Rule):** ${sl:,.2f}
💰 **Take Profit Target:** ${tp:,.2f}

🛡️ *Discipline Mode:* No emotional trailing, pure mathematical setup!"""
        
        bot.reply_to(message, reply_text)
    else:
        bot.reply_to(message, "Bhai, market data fetch karne mein chota glitch aaya. Dobara /setup dabao!")
...
Bot ab iska result monitor karega. 
Jab trade close karni ho, `/result` dabakar check karna ki profit hua ya loss!""")
    else:
